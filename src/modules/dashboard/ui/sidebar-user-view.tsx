...
import { ChevronDown, DollarSignIcon, LogOut, Settings, Triangle, User } from "lucide-react";
          ...
          <Button className="w-full flex items-center justify-start px-3 gap-3 bg-custom-gradient py-4 rounded-lg hover:bg-muted transition">
            {data.user.image ? (
              <Avatar className="h-8 w-8">
                ...
                <GeneratedAvatar seed={data.user.email || data.user.name || "U"} variant="botttsNeutral" className="h-8 w-8 rounded-full" />
            )}

            <div className="flex flex-col text-left">
              <span className="text-[16px] font-bold text-[#24343a]">{data.user.name}</span>

    ...
    <div className="flex flex-col gap-y-4">
      <Tabs defaultValue="summary">
        <div className="bg-white rounded-lg border px-3 ">
          <ScrollArea>
            <TabsList className="bg-background justify-start gap-4 rounded-none h-12">
              <TabsTrigger
                value="summary"
                className="text-muted-foreground rounded-none bg-background data-[state=active]:shadow-none
                 border-b-2 border-transparent data-[state=active]:border-b-[#3ea6ec]
        ...
        <TabsContent value="summary">
          <div className="bg-white flex flex-col gap-4 rounded-lg p-4 border">
            <h2 className="text-xl font-bold capitalize">{data.name}</h2>
            <div className="flex gap-2 items-end">
              <Link
                href={`/agents/${data.agent.id}`}
                className="flex items-center pt-2 gap-x-2 underline underline-offset-4 capitalize"
              >
                <GeneratedAvatar
            ...
            <div>
              <Markdown
                components={{
                  h1: ({ ...props }) => (
                    <h1 className="text-2xl font-bold mb-6" {...props} />
                  ),
                  h2: ({ ...props }) => (
                    <h2 className="text-xl font-bold mb-6" {...props} />
                  ),
                  h3: ({ ...props }) => (
                    <h3 className="text-lg font-bold mb-6" {...props} />
                  ),
                  h4: ({ ...props }) => (
                    <h4 className="text-base font-bold mb-6" {...props} />
                  ),
                  p: ({ ...props }) => (

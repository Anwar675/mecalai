...
const pricingCardvariants = cva("rounded-lg p-4 py-6 w-full", {
  variants: {
    variant: {
      ...
      highlighted: "bg-sidebar text-white",
    },
  },
  defaultVariants: {
    variant: "default",
...
const pricingIconCardvariants = cva("size-5", {
  variants: {
    variant: {
      default: "fill-green-700 text-white ",
            ...
            <ul className={cn("flex flex-col gap-y-2.5", pricingCardSecondaryTextVariants({variant}))}>
                {features?.map((feature, index) => (
                    <li key={index} className="flex items-center gap-x-2.5">
                        <CircleCheckIcon className={cn(pricingIconCardvariants({variant}))} />
                        {feature}
                    </li>
                ))}
            </ul>
